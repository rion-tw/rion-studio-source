...
#include "03_workspace_divider_views.mm"
NS_ASSUME_NONNULL_BEGIN
...
- (nullable id)accessibilityParent {
  id parent = self.tabAccessibilityParent ?: [super accessibilityParent];
...
- (void)accessibilityPerformAction:(NSAccessibilityActionName)action {
  ...
  if ([action isEqualToString:NSAccessibilityDeleteAction]) {
    [self performAccessibilityClose];
    return;
  }
...
- (BOOL)accessibilityDeleteCustomAction {
  return [self performAccessibilityClose];
}
  ...
  return @[
    ...
    [[NSAccessibilityCustomAction alloc]
        initWithName:NSAccessibilityActionDescription(
                         NSAccessibilityDeleteAction)
              target:self
            selector:@selector(accessibilityDeleteCustomAction)],
    [[NSAccessibilityCustomAction alloc]
        initWithName:NSAccessibilityActionDescription(
                         NSAccessibilityIncrementAction)
              target:self
...
- (BOOL)tabCloseButtonHidden {
  return _hideTabCloseButton;
}
...
- (void)configureWithTab:(RionRuntimeTabModel *)tab
  ...
  self.accessibilityIdentifier = [NSString
      stringWithFormat:@"com.rionstudio.runtime.appkit-tab.v1:%@", tab.identifier];
  self.activeTab = tab.active;
  _closeLabel = [closeLabel copy];
  [self setHideTabCloseButton:hideTabCloseButton];
...
- (void)setHideTabCloseButton:(BOOL)hideTabCloseButton {
  ...
  NSString *closeLabel = _closeLabel.length > 0 ? _closeLabel : @"Stop and close tab";
