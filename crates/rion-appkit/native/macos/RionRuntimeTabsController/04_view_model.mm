...
- (nullable id)accessibilityParent {
  id parent = [super accessibilityParent];
...
- (NSDragOperation)draggingUpdated:(id<NSDraggingInfo>)sender {
  ...
  if (parts) {
    ...
    if (reorderedLocalTab) {
      ...
      // the real tab and its browser content surface into this host.
...
@implementation RionRuntimeTabsController {
  ...
  RionRuntimeTabGroupView *_tabCanvas;
  ...
  id _workspaceDividerEventMonitor;
  __weak RionRuntimeWorkspaceDividerView *_activeWorkspaceDivider;
...
- (nullable instancetype)initWithWindow:(NSWindow *)window
  ...
  _previousWindowAccessibilityIdentifier =
      [window.accessibilityIdentifier copy];
  window.accessibilityIdentifier = [NSString
      stringWithFormat:@"com.rionstudio.runtime.appkit-window.v1:%@", _windowID];
  ...
  _workspaceDividerViews = [NSMutableDictionary dictionary];
  ...
  _tabCanvas = [[RionRuntimeTabGroupView alloc] initWithFrame:NSZeroRect];
  root.tabAccessibilityGroup = _tabCanvas;
  _tabCanvas.titlebarAccessibilityParent = root;
  ...
  _tabShortcutMonitor = [NSEvent
    ...
    // Observe physical input only. Chromium owns ordinary key delivery and
    // AppKit retains its normal responder and menu routing.
    return event;
  }];
