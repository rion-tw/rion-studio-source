...
- (void)emitWindowFocusObservation {
  if (_destroyed || !_actionHandler || _windowID.length == 0) return;
  _actionHandler(@{
    @"type" : @"windowFocusChanged",
    @"sourceWindowId" : _windowID,
    @"focused" : @(_window.isKeyWindow),
    @"minimized" : @(_window.isMiniaturized),
    @"visible" : @(_window.isVisible)
  });
}
  ...
  for (NSNotificationName name in names) {
    id observer = [center addObserverForName:name
      ...
      if ([notification.name isEqualToString:NSWindowDidResizeNotification] ||
        ...
        [strongSelf emitWindowFocusObservation];
      } else if ([notification.name
...
- (void)installPreparedToolbarForFullScreen {
  ...
  // Keep the toolbar logically visible so AppKit can place it in its native
  // fullscreen companion host. AutoHideToolbar owns whether that host is on
  // screen; an explicitly hidden NSToolbar cannot be revealed at the edge.
  _fullscreenToolbar.visible = YES;
...
- (void)layoutTitlebarContent {
  ...
  // Liquid Glass may promote the visual tab contents through a private
  // titlebar hierarchy. Keep the semantic AX order explicit and driven by the
  // same native projection, independent of that visual rehosting.
  _tabCanvas.tabAccessibilityChildren = [_tabItems copy];
