...
- (void)setAlwaysHideTabCloseButton:(BOOL)alwaysHide {
  if (_destroyed || _alwaysHideTabCloseButton == alwaysHide) return;
  _alwaysHideTabCloseButton = alwaysHide;
  for (RionRuntimeTabItemView *item in _tabItems) {
    [item setHideTabCloseButton:alwaysHide];
  }
  [self layoutTitlebarContent];
}

- (BOOL)alwaysHideTabCloseButton {
  return _alwaysHideTabCloseButton;
}
...
- (void)updateTabMetadata:(RionRuntimeTabModel *)tab
  ...
  [self setAlwaysHideTabCloseButton:hideTabCloseButton];
  ...
  [item configureWithTab:tab
      ...
      hideTabCloseButton:_alwaysHideTabCloseButton
  ...
  *geometry = {};
  ...
  NSRect windowNameFrame = _windowNameField.hidden
      ...
      : [_windowNameField.superview convertRect:_windowNameField.frame
                                         toView:nil];
  NSRect rootScreenFrame = [root.window convertRectToScreen:rootFrame];
  NSRect tabScreenFrame = [surface.window convertRectToScreen:tabFrame];
  NSRect windowNameScreenFrame = _windowNameField.hidden
      ? NSZeroRect
      : [_windowNameField.window convertRectToScreen:windowNameFrame];
  NSPoint rootTopLeft = RionTopLeftScreenPoint(
      NSMakePoint(NSMinX(rootScreenFrame), NSMaxY(rootScreenFrame)));
  NSPoint tabTopLeft = RionTopLeftScreenPoint(
      NSMakePoint(NSMinX(tabScreenFrame), NSMaxY(tabScreenFrame)));
  CGFloat trafficLightsMaxX = 0;
  NSRect fullscreenControlScreenFrame = NSZeroRect;
  for (NSNumber *buttonType in @[
         @(NSWindowCloseButton),
         @(NSWindowMiniaturizeButton),
         @(NSWindowZoomButton)
    ...
    trafficLightsMaxX = MAX(trafficLightsMaxX, NSMaxX(screenFrame));
    if (buttonType.integerValue == NSWindowZoomButton) {
      NSPoint topLeft = RionTopLeftScreenPoint(
          NSMakePoint(NSMinX(screenFrame), NSMaxY(screenFrame)));
      fullscreenControlScreenFrame = NSMakeRect(
          topLeft.x, topLeft.y, NSWidth(screenFrame), NSHeight(screenFrame));
    }
  }

  // Desktop E2E injects real CGEvents, whose global coordinate space has its
  // origin at the desktop's top-left. Export every horizontal fence and the
  // first rendered tab in that same space so no test needs to guess AppKit's
  // titlebar offsets or translate across multiple displays.
  geometry->rootMinX = rootTopLeft.x;
  geometry->rootWidth = NSWidth(rootScreenFrame);
  geometry->tabMinX = tabTopLeft.x;
  geometry->tabMinY = tabTopLeft.y;
  geometry->tabMaxX = tabTopLeft.x + NSWidth(tabScreenFrame);
  geometry->tabMaxY = tabTopLeft.y + NSHeight(tabScreenFrame);
  geometry->windowNameMaxX = _windowNameField.hidden
      ? 0
      : NSMaxX(windowNameScreenFrame);
  geometry->trafficLightsMaxX = trafficLightsMaxX;
  geometry->fullscreenControlMinX = NSMinX(fullscreenControlScreenFrame);
  geometry->fullscreenControlMinY = NSMinY(fullscreenControlScreenFrame);
  geometry->fullscreenControlWidth = NSWidth(fullscreenControlScreenFrame);
  geometry->fullscreenControlHeight = NSHeight(fullscreenControlScreenFrame);
  geometry->titleHidden = _window.titleVisibility == NSWindowTitleHidden;
  geometry->valid = NSWidth(rootFrame) > 0 && NSWidth(tabFrame) > 0 &&
      NSHeight(tabFrame) > 0 && NSWidth(fullscreenControlScreenFrame) > 0 &&
      NSHeight(fullscreenControlScreenFrame) > 0;
...
- (BOOL)performAccessibilityShowMenuForTabIdentifier:(NSString *)tabIdentifier {
  ...
  // Liquid Glass can promote the item through a private titlebar hierarchy,
  // where `item.window` is not a stable ownership readback. Fence the exact
  // projected item/surface and the retained accessory's live NSWindow instead.
  if (!surface || surface.hidden || !surface.superview ||
      ![_tabItems containsObject:item] ||
      ![_tabSurfaces containsObject:surface] ||
      _accessoryController.view.window != _window) {
    return NO;
  }
...
- (BOOL)matchesTabIdentifiers:(NSArray<NSString *> *)tabIdentifiers
  ...
  NSString *actualActive = _activeTabItem.tabIdentifier;
  return (actualActive == nil && activeTabIdentifier == nil) ||
      [actualActive isEqualToString:activeTabIdentifier];
}

- (BOOL)matchesTabPhases:(NSDictionary<NSString *, NSString *> *)phases {
  if (_destroyed || phases.count != _tabModelsByIdentifier.count) return NO;
  for (NSString *tabId in phases) {
    NSString *phase = phases[tabId];
    RionRuntimeTabModel *tab = _tabModelsByIdentifier[tabId];
    if (![phase isKindOfClass:NSString.class] || ![tab.phase isEqualToString:phase]) return NO;
  }
  return YES;
}
...
- (void)ensureTabIdentifier:(NSString *)tabIdentifier
  ...
  if (existingIndex == NSNotFound) {
    ...
    tab.phase = phase.length > 0 ? phase : @"activating";
  ...
  RionRuntimeTabItemView *item = _tabItemsByIdentifier[tabIdentifier];
  RionRuntimeTabModel *tab = _tabModelsByIdentifier[tabIdentifier];
  tab.name = name.length > 0 ? name : tabIdentifier;
  tab.phase = phase.length > 0 ? phase : @"activating";
  tab.type = type.length > 0 ? type : @"role";
  tab.workspaceTemplate = workspaceTemplate;
