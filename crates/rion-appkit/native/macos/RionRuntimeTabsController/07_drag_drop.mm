...
- (void)showExternalDragGhostForTabIdentifier:(NSString *)tabIdentifier
  ...
  if (sameIdentifier &&
      [_externalDragGhostTabIdentifier isEqualToString:tabIdentifier] &&
      std::fabs(_externalDragGhostWidth - boundedWidth) < 0.5) {
    return;
  }
  _externalDragGhostBeforeIdentifier = [identifier copy];
  _externalDragGhostTabIdentifier = [tabIdentifier copy];
...
- (void)prepareForFullscreenTransition:(BOOL)fullScreen {
  ...
  if (fullScreen) {
    ...
    // NSToolbar remains logically visible in both fullscreen policies.
    // AppKit's AutoHideToolbar presentation option moves its companion host
    // offscreen and restores it from the physical top-edge gesture.
...
- (void)scheduleFullscreenHostRefresh {
  ...
  dispatch_block_t refresh = dispatch_block_create(
      (dispatch_block_flags_t)0, ^{
        ...
        BOOL shouldPin = RionShouldPinFullscreenToolbar(
            strongSelf.alwaysShowInFullScreen, strongSelf.revealLocked);
        ...
        if (shouldPin) {
          ...
          [strongSelf removeTrafficLightObservationRestoringState:YES];
          // The settled host can replace its earlier transition options. Apply
          // auto-hide again after rehosting without forcing the accessory row
          // on screen; AppKit remains responsible for top-edge reveal.
          [strongSelf updateFullscreenToolbarPresentationPolicy];
        }
...
- (void)applyFullScreenPolicy {
  ...
  if (fullScreen) {
    ...
    if (RionShouldPinFullscreenToolbar(self.alwaysShowInFullScreen,
                                       self.revealLocked)) {
      // Keep the root content full-size while the native row is pinned.
    ...
    _toolbar.visible = YES;
    [self synchronizeFullScreenTitlebarGeometry];
    [self removeTrafficLightObservationRestoringState:YES];
    [self scheduleContentLayoutNotification];
    return;
  }
