...
fn schema_twenty_four_migrates_game_window_role_slots_with_stable_fallbacks() {
    ...
    assert_eq!(
        migrated["tabs"][1]["roleSlots"][0]["slotId"],
        "legacy:0:role-3"
    );
    assert_eq!(
...
#[test]
fn schema_twenty_seven_adds_complete_workspace_slots_and_resets_legacy_web_content() {
    let connection = Connection::open_in_memory().unwrap();
    create_schema(&connection, false).unwrap();
    ...
    let workspace = json!({
        ...
        "slots": [
            ...
            {
                "id": "slot-web",
                "web": {"lastUrl":"https://example.test/"},
...
fn schemas_nineteen_through_twenty_two_remove_only_retired_sync_fields() {
    for schema in 19..=22 {
        ...
        .unwrap();
        assert!(
            migrated_role.get("localStorageSourceRoleId").is_none(),
            "schema {schema}"
        );
...
fn source_macro_survives_sqlite_reopen_and_old_macros_keep_fixed_targets() {
    ...
    let path = directory.path().join("source-macro.sqlite");
    {
        ...
        let record = |id: &str, mode: Option<&str>| {
            ...
            if let Some(mode) = mode {
                value["executionMode"] = json!(mode);
                value["shortcutSourceScope"] = json!({"type":"all_roles"});
            }
            value
        };
    ...
    assert!(matches!(
        source.shortcut_source_scope,
        crate::model::MacroShortcutSourceScope::AllRoles
    ));
}
