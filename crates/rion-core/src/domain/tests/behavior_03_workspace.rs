...
fn workspace_store_domain_contracts() {
...

    {
        let mut workspaces = Vec::new();
        let created =
            create_workspace(&mut workspaces, workspace_input(json!({"name":"Default"}))).unwrap();
...

    {
        ...
        let created = create_workspace(
            ...
            workspace_input(json!({
                "name":"Web apps",
                "slots":[{
                    "web":{"lastUrl":"https://www.youtube.com"},
        ...
        assert_eq!(web.last_url.as_deref(), Some("https://www.youtube.com/"));
        ...
        let invalid_url = create_workspace(
            ...
            workspace_input(json!({
                "name":"Invalid URL",
                "slots":[{"web":{"lastUrl":"file:///tmp/test"}}]
            })),
        )
        .unwrap_err();
        ...
        let outside_layout = create_workspace(
            ...
            workspace_input(json!({
                "name":"Outside",
                "template":"single",
                "slots":[{}, {"web":{"lastUrl":"https://example.com"}}]
            })),
        )
        .unwrap_err();
...

    {
        let mut workspaces = Vec::new();
        let first =
            create_workspace(&mut workspaces, workspace_input(json!({"name":"First"}))).unwrap();
        let second =
            create_workspace(&mut workspaces, workspace_input(json!({"name":"Second"}))).unwrap();
        ...
        let third =
            create_workspace(&mut workspaces, workspace_input(json!({"name":"Third"}))).unwrap();
...

    {
        let mut workspaces = Vec::new();
        let created =
            create_workspace(&mut workspaces, workspace_input(json!({"name":"Layout"}))).unwrap();
...

    {
        ...
        assert!(
            create_workspace(&mut workspaces, workspace_input(json!({"name":" party "}))).is_err()
        );
        assert!(
            create_workspace(
                &mut workspaces,
...

    {
        let mut workspaces = Vec::new();
        let workspace =
            create_workspace(&mut workspaces, workspace_input(json!({"name":"Order"}))).unwrap();
        for order in [
            vec![],
...
fn workspace_start_page_is_normalized_and_never_changes_existing_urls() {
    let mut workspaces = Vec::new();
    let entrance = create_workspace(
        &mut workspaces,
        workspace_input(json!({
            "name": "Entrance", "template": "single", "slots": [{"web": {}}]
        })),
    )
    .unwrap();
    assert_eq!(
        entrance.slots[0].web.as_ref().unwrap().launch_url(),
        "rion-start://home/"
    );

    let resumed = create_workspace(
        &mut workspaces,
        workspace_input(json!({
            "name": "Resumed", "template": "single",
            "slots": [{"web": {"lastUrl": " https://example.test/path?q=1#part "}}]
        })),
    )
    .unwrap();
    assert_eq!(
        resumed.slots[0].web.as_ref().unwrap().last_url.as_deref(),
    ...
    assert!(
        create_workspace(
            &mut workspaces,
            workspace_input(json!({
                "name": "Invalid", "slots": [{"web": {"lastUrl": "rion-start://home/"}}]
            }))
        )
        .is_err()
    );
