...
mod tests {
    ...
    fn resolves_adaptive_zoom_with_hysteresis() {
        ...
        assert_eq!(adaptive_zoom_percent(f64::NAN, Some(67)), 67);
    }

    #[test]
    fn normalizes_touching_rect_edges_to_one_authoritative_value() {
        let rects = normalize_rect_edges(&[
            LayoutRect {
                x: 0.0,
                y: 0.0,
                width: 0.333_333_333,
        ...
        assert_eq!(rects[1].x, 0.3333);
        assert_eq!(rects[1].width, 0.6667);
    }
