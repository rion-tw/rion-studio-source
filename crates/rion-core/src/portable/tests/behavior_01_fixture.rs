...
fn runtime_authority_fixture_uses_the_current_portable_contract() {
    let source = include_str!("../../../../../tests/fixtures/runtime-authority/portable.json");
    ...
    let dormant_workspace = data
        ...
        .unwrap();
    assert!(data.game_windows.iter().all(|window| {
        window
            .tabs
            .iter()
...
fn rejects_cycles_and_unsupported_versions() {
    ...
    let future = fixture(11).replace(
        "\"schemaVersion\":11",
        &format!("\"schemaVersion\":{}", PORTABLE_SCHEMA_VERSION + 1),
    );
...
fn source_role_macro_portable_round_trip_without_a_shortcut() {
    ...
    assert_eq!(
        data.macros[0].execution_mode,
        Some(crate::model::MacroExecutionMode::SourceRole)
    );
    assert!(matches!(
        data.macros[0].shortcut_source_scope,
...
fn workspace_start_page_portable_roundtrip_and_legacy_import() {
    ...
    let mut current = fixture_value(22);
    current["launchWorkspaces"][0]["slots"][0]
        .as_object_mut()
        .unwrap()
        .remove("roleId");
    current["launchWorkspaces"][0]["slots"][0]["web"] =
        json!({"lastUrl":"https://example.test/path?q=1#part"});
    let normalized = normalize(&current.to_string()).unwrap();
    assert_eq!(
        normalized["launchWorkspaces"][0]["slots"][0]["web"]["lastUrl"],
