...
Rion Studio は、Web ゲームのプレイヤーが各ロール、ブラウザーセッション、ウィンドウ配置を 1 つのデスクトップ App で整理できるようにします。専用のブラウザーロールを作成し、ゲーム画面を直接開き、Chrome セッションを取り込み、プレイを自分で管理しながら反復的な手作業を減らせます。
...
- 必要に応じて、Chrome から既存のブラウザーセッションを取り込めます。
...
Chrome を終了して同意した後、選択したブラウザー保存領域をロールへコピーし、Cookie を埋め込みセッションへ注入します。パスワード、自動入力、履歴、ブックマーク、拡張機能は取り込みません。外部 Chrome はゲーム互換モードとしてのみ残ります。
