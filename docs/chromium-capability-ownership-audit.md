...
| Popups | Opener `setWindowOpenHandler` synchronously returns Electron `allow` with fixed security overrides; Electron creates the original BrowserWindow/WebContents and `did-create-window` hands it to ChromiumPopupLifecycleCoordinator. Exact Session, native parent, connected/noopener frame, owner generation, Core admission, bounds/zoom, and native-ready receipt fence visibility. No stop/reload/adoption path exists; nested opens are denied. | Popup lifecycle and Role/global-Web registry suites cover named target, POST metadata, early close/load, mismatch, stale admission and native release. POPUP-012 uses `electronBrowserWindow` on both platforms; the macOS parent remains AppKit. |
