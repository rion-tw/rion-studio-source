...
| Popups | `setWindowOpenHandler` performs bounded synchronous preflight, then its `createWindow` callback returns the WebContents of a hidden Electron BrowserWindow. The first remote main-frame navigation is prevented until Core admits the exact parent/session/opener fence; the same WebContents then loads POST/referrer data and is shown only after `nativeReady`. Connected opener and explicit noopener are separate contract values. Nested opens are denied. | Popup lifecycle tests use the iQIYI `thirdLoginWindow` feature shape and cover opener mismatch, stale admission and native release. Paired POPUP-012 journeys require `electronBrowserWindow` on both platforms while macOS keeps the parent AppKit host. |
