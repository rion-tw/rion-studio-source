...
The paired `chromium-workspace-gap-dividers-seed` / `restart` phases create a
`main_left_stack_right` Workspace with two Roles and one Website, open it with a
1px gap, and then select 16px through the visible Settings UI while the native
host remains live. Read-only evidence proves the exact horizontal and vertical
surface gaps, stable window generation and topology revision for the appearance
change, and a higher native projection revision. The primary divider actions are
real CGEvents against AppKit splitters or WebDriver pointer streams against the
Windows bundled-host separators, selected by both axis and Core divider index.
Both terminal moves must update and persist only the Core-owned slot rects, and
restart must restore their layout with the 16px setting and no shell errors.
...
popup opened by a real Shift+left click. Both platforms require the same
`electronBrowserWindow` popup host, exact Session/opener and native hostname
title; macOS additionally proves that the parent retains its exact AppKit
identity. A named OAuth fixture crosses origin, returns to a same-origin
callback, delivers real storage/postMessage signals and script-closes after the
parent renders completion. Both website exit and Escape restore the paired
