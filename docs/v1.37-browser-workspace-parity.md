...
- Current classifications: **1** direct tests, **182** assertion-evidenced replacements, **43** intentional System WebView-only changes.
...
- **154** of the 187 stale mappings now point to an exact executable test plus a unique assertion/case ID.
- The remaining **33** stale mappings are intentional retirements, not claims of current parity.

## Intentional v1 retirements

- **43** behaviors depend on Electron, External Chrome, custom proxy, or superseded runtime policy that the System WebView-only architecture intentionally removed.
