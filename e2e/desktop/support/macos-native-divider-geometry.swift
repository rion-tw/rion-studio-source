...
for window in windows {
  ...
  for candidate in descendants(window) where text(candidate, "AXRole") == "AXSplitter" {
    if text(candidate, "AXTitle") == "Resize workspace columns" ||
       text(candidate, "AXDescription") == "Resize workspace columns" {
      matches.append((String(identifier.dropFirst(prefix.count)), candidate))
    }
  }
}
guard matches.count <= 1 else { fail("multiple retained AppKit workspace dividers") }
guard let (windowId, divider) = matches.first else { print("PENDING|no exact native splitter"); exit(0) }
...
let data = try JSONSerialization.data(withJSONObject: [
  "windowId": windowId, "x": point.x, "y": point.y,
