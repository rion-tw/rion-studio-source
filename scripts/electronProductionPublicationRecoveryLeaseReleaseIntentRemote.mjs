...
export async function proveElectronProductionPublicationRecoveryLeaseReleaseIntentHistory(
  input
) {
  assertExactKeys(input, [
    ...
    "target",
    "token"
  ], "publication recovery intent-history remote input");
  ...
  if (target.ref !== target.repositoryPolicy.defaultBranch) {
    throw new Error("The intent-history ref must be the private default branch.");
  }
  ...
  const intentBlobSha = requiredCommitSha(
    input.intentBlobSha,
    "publication recovery intent-history blob SHA"
  );
  ...
  if (openingHead !== currentObservation.headCommitSha) {
    throw new Error("The intent-history opening head is not the reader head.");
  }
  ...
  if (!Array.isArray(pathCommits) || pathCommits.length !== 1 ||
    ...
    throw new Error(
      "The fixed durable intent path must have exactly one creating commit."
    );
  }
  const intentCommitSha = pathCommits[0].sha;
  const intentCommit = await readCommit(
    dependencies,
    target,
    intentCommitSha,
    "intent-create-commit"
  );
  if (intentCommit.parentCommitSha !== initialHeadCommitSha) {
    throw new Error("The intent create commit does not directly follow H0.");
  }
  const comparison = await readJson(
    dependencies,
    `${repositoryUrl(target)}/compare/${initialHeadCommitSha}...${intentCommitSha}`,
    "intent-create-diff",
    MAX_COMPARE_BYTES
  );
  assertExactIntentCreateDiff(
    comparison,
    initialHeadCommitSha,
    intentCommitSha,
    target.path,
  ...
  if (closingHead !== openingHead) {
    throw new Error("The intent-history head changed during observation.");
  }
  return createElectronProductionPublicationRecoveryLeaseReleaseIntentHistory({
    target: {
    ...
    initialHeadCommitSha,
    intentCommitSha,
    intentTreeSha: intentCommit.treeSha,
    intentBlobSha,
    currentObservation,
    pathHistory: {
      reachableFromHeadCommitSha: openingHead,
      commitSha: intentCommitSha,
      resultCount: 1,
      nextPage: false
    },
    observedAt
...
async function readHead(dependencies, target, stage) {
  ...
  if (!isObject(reference) || reference.ref !== `refs/heads/${target.ref}` ||
    ...
    throw new Error(`The intent-history ${stage} reference is malformed.`);
  }
...
async function readCommit(dependencies, target, sha, stage) {
  ...
  if (!isObject(value) || value.sha !== sha || !isObject(value.tree) ||
    ...
    throw new Error(`The intent-history ${stage} commit is not linear.`);
  }
...
function assertExactIntentCreateDiff(value, initialHead, intentCommit, intentPath,
  intentBlobSha) {
  if (!isObject(value) || value.status !== "ahead" || value.ahead_by !== 1 ||
      ...
      !isObject(value.commits[0]) || value.commits[0].sha !== intentCommit ||
      !Array.isArray(value.files) || value.files.length !== 1) {
    throw new Error("The intent create comparison is not one exact commit.");
  }
  const file = value.files[0];
  if (!isObject(file) || file.filename !== intentPath ||
      file.status !== "added" || file.sha !== intentBlobSha) {
    throw new Error("The intent create commit changed a foreign path or blob.");
  }
}

function assertExpectedObservation(value) {
  assertExactKeys(value, ["headCommitSha", "parentCommitShas", "treeSha"],
    "publication recovery intent-history expected observation");
  if (!Array.isArray(value.parentCommitShas) ||
      value.parentCommitShas.length !== 1) {
    throw new Error("The intent-history expected head must be linear.");
  }
  return Object.freeze({
    headCommitSha: requiredCommitSha(value.headCommitSha,
      "intent-history expected head SHA"),
    treeSha: requiredCommitSha(value.treeSha,
      "intent-history expected tree SHA"),
    parentCommitShas: [requiredCommitSha(value.parentCommitShas[0],
      "intent-history expected parent SHA")]
  });
}
  ...
  try {
    response = await dependencies.fetchImpl(url, {
      ...
      headers: {
        ...
        "User-Agent": "rion-studio-recovery-intent-history",
        "X-GitHub-Api-Version": "2022-11-28"
      },
      redirect: "error"
    });
  } catch (error) {
    throw new Error(`Recovery intent-history ${stage} transport failed.`, {
      cause: error
    });
  }
  if (!response || response.status !== 200) {
    await cancelResponse(response);
    throw new Error(`Recovery intent-history ${stage} was not acknowledged.`);
  }
...
async function readBoundedJson(response, stage, maximumBytes) {
  ...
  if (declaredLength !== null && declaredLength !== undefined &&
    ...
    await cancelResponse(response);
    throw new Error(`Recovery intent-history ${stage} exceeds its byte bound.`);
  }
  if (!response.body || typeof response.body.getReader !== "function") {
    throw new Error(`Recovery intent-history ${stage} body is unavailable.`);
  }
  ...
  try {
    ...
    throw new Error(`Recovery intent-history ${stage} body is malformed.`, {
      cause: error
    });
  }
}
...
function hasNextPage(link) {
  ...
  if (typeof link !== "string" || link.length > 8192) {
    throw new Error("The intent-history pagination header is invalid.");
  }
...
function assertRepositoryPolicy(value, target) {
  if (!isObject(value) || value.full_name !== `${target.owner}/${target.repo}` ||
    ...
    throw new Error("The intent-history repository policy does not match.");
  }
}

function assertDependencies(fetchImpl, token) {
  if (typeof fetchImpl !== "function") {
    throw new Error("An intent-history fetch implementation is required.");
  }
  if (typeof token !== "string" || token.length === 0 || token.length > 4096 ||
      /\s/u.test(token)) {
    throw new Error("A bounded intent-history token is required.");
  }
