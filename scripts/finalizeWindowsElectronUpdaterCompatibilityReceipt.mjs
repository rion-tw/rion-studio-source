...
import { finalizeWindowsElectronUpdaterCompatibilityTerminalReceipt } from
...
export async function finalizeWindowsElectronUpdaterCompatibilityReceipt(
  argumentsList
) {
...
function parseOptions(argumentsList) {
  if (!Array.isArray(argumentsList)) {
    throw new Error("The Windows compatibility finalizer arguments are invalid.");
  }
  ...
  while (index < argumentsList.length) {
    ...
    if (
      ...
      throw new Error("The Windows compatibility finalizer arguments are malformed.");
    }
    const name = option.slice(2);
    if (!OPTIONS.includes(name)) {
      throw new Error("The Windows compatibility finalizer option is unknown.");
    }
    if (values.has(name)) {
      throw new Error("The Windows compatibility finalizer option is duplicated.");
    }
    values.set(name, value);
    index += 2;
  }
  if (values.size !== OPTIONS.length) {
    throw new Error("The Windows compatibility finalizer options are incomplete.");
  }
  return values;
}
...
if (
  ...
  try {
    await finalizeWindowsElectronUpdaterCompatibilityReceipt(
      process.argv.slice(2)
    );
    process.stdout.write(
      "Finalized Windows Electron updater compatibility receipt.\n"
    );
  } catch {
