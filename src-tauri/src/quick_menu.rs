...
fn menu(app: &AppHandle, core: &AppCore) -> Result<Menu<tauri::Wry>, String> {
    ...
    if role_values.is_empty() {
        ...
        for role in role_values {
            if let (Some(id), Some(name)) = (role["id"].as_str(), role["name"].as_str()) {
                role_menu = role_menu.text(format!("{ROLE_PREFIX}{id}"), name);
            }
        }
    }
    ...
    if workspace_values.is_empty() {
        ...
        for workspace in workspace_values {
            if let (Some(id), Some(name)) =
                (workspace["id"].as_str(), workspace["name"].as_str())
            {
                workspace_menu =
                    workspace_menu.text(format!("{WORKSPACE_PREFIX}{id}"), name);
            }
        }
    }
    ...
    MenuBuilder::new(app)
        .text("open-app", "Open Rion Studio")
