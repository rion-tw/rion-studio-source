...
fn desktop_e2e_windows_minimize_screen_rect(
  ...
  if (!element || element.hidden || element.getClientRects().length !== 1) return null;
  ...
  return { left: rect.left, top: rect.top, right: rect.right, bottom: rect.bottom };
        ...
        "The WebView2 minimize control is not visible.",
        "The WebView2 minimize-control bounds readback did not complete.",
    )
}
