...
fn flyff_china_identity_integrity_runs_even_when_every_sync_selector_is_disabled() {
    ...
    assert!(observer.contains("parseFlyffChinaLengthPrefixed(lines[8], 65536)"));
    assert!(!observer.contains("flyff_chinaInteger"));
    assert!(!observer.contains("validFlyffChinaBindings"));
}
...
fn flyff_china_v7_selector_capture_and_merge_preserve_identity_and_unknown_fields() {
    let selectors = rion_core::FLYFF_CHINA_LOCAL_STORAGE_SYNC_SELECTORS
        ...
        .collect::<Vec<_>>();
    let source_value =
        flyff_china_codec_reference::fixture(SOURCE_FLYFF_CHINA_IDENTITY, "0.5");
    let target_value = flyff_china_codec_reference::fixture(TARGET_FLYFF_CHINA_IDENTITY, "1");
...
fn flyff_china_v7_codec_fails_closed_for_unknown_or_malformed_payloads() {
    ...
    let malformed_identity = valid.replacen(
        &format!("40 {SOURCE_FLYFF_CHINA_IDENTITY}"),
        "3 bad",
        1,
    );
    assert!(flyff_china_codec_reference::parse(&malformed_identity).is_err());
...
#[test]
fn flyff_china_v7_selector_fields_require_exact_bounded_line_mappings() {
    ...
    assert!(validate_flyff_china_selector_entries(&selectors, &valid).is_ok());
    ...
    for invalid in [
        ...
        assert!(validate_flyff_china_selector_entries(&selectors, &invalid).is_err());
    }
}
