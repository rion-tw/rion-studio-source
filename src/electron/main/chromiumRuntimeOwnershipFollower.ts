import { handoffChromiumContentFocus, requestChromiumContentFocus } from "./chromiumRuntimeContentFocus";
...
export async function followChromiumRuntimeOwnership(
  ...
  try {
    ...
    if (transitions.length === 0) {
      ...
      if (!input.signal?.aborted) handoffChromiumContentFocus(input, input.projectedWindows, input.projectedRoles);
      return undefined;
    }
  ...
  return coreEffectEventContinuation(
    continuation.completion.then((receipt) => {
      if (receipt.status === "applied") {
        ...
        if (focusedTab) requestChromiumContentFocus(input, focusedTab.specification.tabId);
        if (!input.signal?.aborted) handoffChromiumContentFocus(input, input.projectedWindows, input.projectedRoles);
      }
      return receipt;
    }),
