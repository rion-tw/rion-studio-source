...
describe("Chromium Workspace cutover paired replacements", () => {
  ...
  it("accepts only the hidden activating projection sampled during visible reopen", async () => {
    ...
    auxiliary.windowId = "00000000-0000-4000-8000-000000000042";
    auxiliary.appKitIdentity.logicalWindowId = auxiliary.windowId;
    auxiliary.web.tabId = auxiliary.tabId;
    ...
    await expect(validateWebOnlyHistory([
      ready, degraded, activating, recovered, auxiliary
  ...
  it("uses visible controls while preserving retained AppKit and exact Core fences", async () => {
    ...
    for (const marker of [
      ...
      "openVisibleLanguagePreferences",
      "optionLabel: \"繁體中文\"",
      "language: \"zh-TW\"",
