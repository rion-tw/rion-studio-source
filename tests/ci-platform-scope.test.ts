...
const matrices = [...workflow.matchAll(/include: \$\{\{ fromJSON\(github.event_name == 'workflow_dispatch' && inputs.platform_scope == 'macos' && '(\[[^']+\])' \|\| '(\[[^']+\])'\) \}\}/gu)];

describe("manual macOS CI scope", () => {
  it.each([
    ...
    for (const match of matrices) {
      const selected = event === "workflow_dispatch" && scope === "macos" ? match[1]! : match[2]!;
