...
import { resolveDesktopE2eProfileName, resolveDesktopE2eRuntimeTarget } from
...
describe("desktop E2E runtime-target execution plan", () => {
  it("rejects a retired Tauri driver instead of launching it", () => {
    expect(() => resolveDesktopE2eRuntimeTarget({
      architecture: "arm64", manifest, platform: "darwin",
      profileName: "compatibility", repositoryRoot: root
    })).toThrow("unsupported driver tauri");
  });

  it.each([
    ["darwin", "chromium-macos-appkit"],
    ["win32", "chromium-windows"]
  ] as const)("maps default profiles to complete Electron profiles on %s", (platform, prefix) => {
    for (const profileName of ["smoke", "full"]) {
      expect(resolveDesktopE2eProfileName({ platform, profileName })).toBe(prefix + "-smoke");
    }
    expect(resolveDesktopE2eProfileName({ platform, profileName: "extended" }))
      .toBe(prefix + "-hardware-extended");
  });

  it("rejects default desktop profiles on an unsupported host", () => {
    expect(() => resolveDesktopE2eProfileName({ platform: "linux", profileName: "full" }))
      .toThrow("does not support host platform linux");
  });
