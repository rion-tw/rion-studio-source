...
import { type ChromiumRuntimeExistingTabActivationFence } from "../src/electron/main/chromiumRuntimeLaunchCoordinator";
...
describe("Electron Chromium runtime launch coordinator", () => {
  ...
  it("reuses exact tab chrome while the mixed workspace Web surface is still loading", async () => {
    const { coordinator, launchCommands, state } = launchHarness({
      onLaunch: (command, harness) => {
        if (command.type === "browserWorkspaceLaunch") {
          configureWorkspaceWebLaunch(harness, {
  ...
  it.each([true, false])("distinguishes retained cleanup from an unknown extra tab (retiring=%s)", async retiring => {
    const { coordinator, state } = launchHarness({ observedSnapshots: true });
    await coordinator.launchRole(ROLE_ID, { kind: "new-window" });
    state.nativeSnapshot = { ...state.nativeSnapshot, tabs: [...state.nativeSnapshot.tabs,
      { ...state.nativeSnapshot.tabs[0]!, tabId: "retired-tab", ...(retiring ? { retiring: true as const } : {}) }
    ] };
    const launch = coordinator.launchWorkspace(WORKSPACE_ID, { kind: "game-window", windowId: WINDOW_ID });
    if (retiring) await expect(launch).resolves.toMatchObject({ windowId: WINDOW_ID });
  ...
  ] as const)("uses current %s for an already admitted native tab", async (
    _identity,
    mutateIdentity
  ) => {
    let admitted = false;
