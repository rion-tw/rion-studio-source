...
describe("event topology architecture contract", () => {
  it("documents event-bound completion as the default and contract version 19", async () => {
    const [agents, policy, runtimeContract, runtimeOperations] = await Promise.all([
      ...
      readFile(
        new URL("../docs/contracts/system-runtime/operations-and-receipts.md", import.meta.url),
        "utf8"
      )
    ]);
    ...
    expect(runtimeContract).toContain("Contract version 19");
