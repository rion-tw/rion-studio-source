...
describe("LaunchWorkspaceStore", () => {
  ...
  it("persists companion geometry and normalizes HTTP shortcuts", async () => {
    const workspace = await store.createWorkspace({
      name: "Watch party",
      companion: {
        placement: "right",
    ...
    expect(workspace.companion).toEqual({
      placement: "right",
      sizePercent: 33,
      autoOpen: true,
      target: { kind: "url", url: "https://example.com/watch" }
  ...
  it("supports every companion edge and size and disables auto-open without a target", async () => {
    const placements = ["left", "right", "top", "bottom"] as const;
    const sizes = [25, 33, 40, 50] as const;
  ...
  it("deep-clones, clears, and validates companion targets", async () => {
    const workspace = await store.createWorkspace({
      name: "Player",
      companion: {
        placement: "left",
    ...
    const listed = await store.listWorkspaces();
    if (listed[0].companion?.target?.kind === "application") {
      listed[0].companion.target.label = "Changed";
    }
    await expect(store.getWorkspace(workspace.id)).resolves.toMatchObject({
      companion: { target: { label: "Player" } }
    });

    const cleared = await store.updateWorkspace(workspace.id, { companion: null });
    expect(cleared).not.toHaveProperty("companion");
    ...
    await expect(store.createWorkspace({
      ...
      companion: {
        ...
        sizePercent: 33,
        autoOpen: false,
        target: { kind: "url", url: "file:///tmp/video" }
      }
    })).rejects.toMatchObject({ code: "WORKSPACE_COMPANION_URL_INVALID" });
    ...
    })).rejects.toMatchObject({ code: "WORKSPACE_COMPANION_APPLICATION_INVALID" });
  });
