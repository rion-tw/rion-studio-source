...
on run
  ...
  it("drives the exact production package without a Chromium debug transport", async () => {
    ...
    expect(profileRunner).toContain("scripts\\windowsJobProcessDiagnostics.cs");
    expect(profileRunner).toContain("RION_WINDOWS_JOB_PROCESS_DIAGNOSTICS=");
    expect(profileRunner).toContain("authoritative = $false");
