...
describe("shared runtime health diagnostics", () => {
  it("exports a bounded privacy-safe native runtime summary", async () => {
    const [core, snapshot, diagnostics, failure, activeFence, fenceEvent, operation, subsystem, status, scope, capabilityEvidence, generatedIndex] =
      await Promise.all([
