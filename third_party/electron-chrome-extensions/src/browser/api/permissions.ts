...
export class PermissionsAPI {
  ...
  private processExtension(extension: Electron.Extension) {
    ...
    this.permissionMap.set(extension.id, {
      permissions: (manifest.permissions || []) as chrome.runtime.ManifestPermission[],
  ...
  private remove = ({ extension }: ExtensionEvent, permissions: chrome.permissions.Permissions) => {
    ...
    const removable = new Set(extension.manifest.optional_permissions || [])
    const removedPermissions = (permissions.permissions || []).filter((permission) =>
      removable.has(permission),
    )
    current.permissions = current.permissions.filter((permission) =>
      !removedPermissions.includes(permission),
    )
    const removableOrigins = new Set(extension.manifest.optional_host_permissions || [])
    const removedOrigins = (permissions.origins || []).filter((origin) =>
      removableOrigins.has(origin),
    )
    current.origins = current.origins.filter((origin) => !removedOrigins.includes(origin))
    const changed = removedPermissions.length > 0 || removedOrigins.length > 0
    if (changed) {
  ...
  private request = async (
    ...
    this.ctx.router.sendEvent(extension.id, 'permissions.onAdded', request)
    return true
  }
}
