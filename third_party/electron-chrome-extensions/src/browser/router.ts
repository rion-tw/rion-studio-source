...
const MAX_HANDLER_NAME_LENGTH = 96
const MAX_EVENT_NAME_LENGTH = 128
const MAX_ARGUMENT_BYTES = 256 * 1024

const validRouteName = (value: unknown, maximum: number) =>
  typeof value === 'string' && value.length > 0 && value.length <= maximum &&
  /^[A-Za-z][A-Za-z0-9.]*$/u.test(value)

const extensionOrigin = (extensionId: string) => `chrome-extension://${extensionId}/`

const assertAuthorizedExtensionSender = (event: IpcAnyEvent, extensionId: string) => {
  const expected = extensionOrigin(extensionId)
  const actual = event.type === 'service-worker'
    ? event.serviceWorker.scope
    : event.senderFrame?.url || event.sender.getURL()
  if (!actual?.startsWith(expected)) throw new Error('RION_EXTENSION_IPC_SENDER_MISMATCH')
}

const assertBoundedArguments = (args: any[]) => {
  let encoded: string
  try {
    encoded = JSON.stringify(args)
  } catch {
    throw new Error('RION_EXTENSION_IPC_ARGUMENT_INVALID')
  }
  if (encoded.length > MAX_ARGUMENT_BYTES) throw new Error('RION_EXTENSION_IPC_ARGUMENT_LIMIT')
}
...
class RoutingDelegate {
  ...
  addObserver(observer: RoutingDelegateObserver) {
    ...
    const maybeListenForWorkerEvents = ({
      ...
      if (
        ...
        d(`listening to extension service worker [versionId:${versionId}]`)
  ...
  private onRouterMessage = async (
    ...
    if (!validRouteName(handlerName, MAX_HANDLER_NAME_LENGTH)) {
      throw new Error('RION_EXTENSION_IPC_HANDLER_INVALID')
    }
    assertAuthorizedExtensionSender(event, extensionId)
    assertBoundedArguments(args)
    d(`received '${handlerName}'`)
  ...
  private onAddListener = (event: IpcAnyEvent, extensionId: string, eventName: string) => {
    if (!validRouteName(eventName, MAX_EVENT_NAME_LENGTH)) return
    assertAuthorizedExtensionSender(event, extensionId)
    const observer = this.sessionMap.get(getSessionFromEvent(event))
...
export class ExtensionRouter {
  ...
  constructor(
    ...
    app.on('web-contents-created', (event, webContents) => {
      if (webContents.session === this.session && webContents.getType() === 'backgroundPage') {
        d(`storing reference to extension background host [id:${webContents.id}]`)
    ...
    session.serviceWorkers.on(
      ...
      ({ runningStatus, versionId }: any) => {
        ...
        if (this.extensionHosts.has(serviceWorker)) {
          d('extension worker running status changed to %s', runningStatus)
        } else {
          d('storing reference to extension background service worker')
  ...
  private observeListenerHost(host: FrameEventListener['host']) {
    ...
    d(`observing extension listener [id:${hostId}]`)
    host.once('destroyed', () => {
